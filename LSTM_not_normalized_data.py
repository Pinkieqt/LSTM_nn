...
class LSTMnn:
    ...
    def __generateY(self, ds):
        Y = []
        for i in range(ds.shape[0]):
            # hardcoded
            # if(self.ds.loc[i, 'D8'] > 17 and self.ds.loc[i, 'D10'] > 42 and self.ds.loc[i, 'D12'] < -42):
            # if(self.ds.loc[i, 'D8'] > 153 and self.ds.loc[i, 'D10'] > 42 and self.ds.loc[i, 'D12'] < 100):
            if(ds.loc[i, 'D8'] > 0.011 and ds.loc[i, 'D10'] > 0.015 and ds.loc[i, 'D12'] < -0.05):
                # anomaly
                Y.append([0, 1])
            else:
                # not anomaly, normal
...
def main():
    ...
    nn = LSTMnn(
        './Final_datasets/iphone_new.csv', lookback, features)
    ...
    if(train):
        ...
        nn.initModel(x_train=x_train, y_train=y_train,
                     dropoutrate=0.1, learningrate=0.000035)  # 0.0001 - 30
    ...
    if(not train):
        ...
        imgNumberToPlot = 2864
        plotImage(prediction[imgNumberToPlot][1],
                  folderSource, predictSource, imgNumberToPlot)

        # Show images animation
        # animatePrediction(prediction, 720, 1020,
